...
```sh
make docker-build docker-push IMG=<some-registry>/slack-notifier-controller:tag
```
...
```sh
make deploy IMG=<some-registry>/slack-notifier-controller:tag
```
...
```sh
make build-installer IMG=<some-registry>/slack-notifier-controller:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/slack-notifier-controller/<tag or branch>/dist/install.yaml
```
